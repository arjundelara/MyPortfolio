...
function new_york_business_fonts_url() {
	...
	if ( 'off' !== $typography ) {
		...
		$font_families[] = wp_strip_all_tags(get_theme_mod('header_fontfamily', 'Oswald')).':300,400,500';
...
function new_york_business_general_admin_notice(){
	...
	if($pagenow == 'index.php' || $pagenow == 'themes.php'){
         $msg = sprintf('<div data-dismissible="disable-done-notice-forever" class="notice notice-info is-dismissible" >
			 	...
			 	<a href="?new_york_business_notice_dismissed" target="_self"  style="text-decoration: none; margin-left:10px;" class="button button-secondary">%11$s</a>
			 	</p></div>',
				esc_html__(' If you like New York Business ','new-york-business'),
				esc_html__(' theme, please leave us a ','new-york-business'),
				esc_html__(' Rating ','new-york-business'),
				esc_html__(' Huge thanks in advance. ','new-york-business'),
				esc_url(new_york_business_THEME_REVIEW_URL),
				esc_html__('Rate','new-york-business'),				
				esc_url(new_york_business_THEME_URL),
				esc_html__('Go Pro Version','new-york-business'),
				esc_url(new_york_business_THEME_DOC),	
				esc_html__('Documentation','new-york-business'),
...
if ( isset( $_GET['new_york_business_notice_dismissed'] ) ){
	update_option('new_york_business_notice', 2);
}

$new_york_business_notice = get_option('new_york_business_notice', 0);
if($new_york_business_notice != 2){
