	...
	class Enlighten_Welcome {
			...
			public function __construct( $plugins, $strings ) {
				...
				$this->tab_sections = array(
					'getting_started' => esc_html__('Getting Started', 'bloger'),
					'actions_required' => esc_html__('Required Plugins', 'bloger'),
					'recommended_plugins' => esc_html__('Recommended Plugins', 'bloger'),
					'demo_import' => esc_html__('Import Demo', 'bloger'),
					'free_vs_pro' => esc_html__('Free Vs Pro', 'bloger'),
					'changelog' => esc_html__('ChangeLog', 'bloger'),
					'support' => esc_html__('Support', 'bloger'),
				);
			...
			public function welcome_admin_notice_display() {
				...
				if( is_admin() && ('themes.php' == $pagenow) /*&& (isset($_GET['activated']))*/ ) {
					...
					<div class="updated apwelcome-an notice notice-success is-dismissible">
						<h2><?php printf(esc_html__( 'Welcome!', 'bloger' )) ?></h2>
						<p class="about-desc"><?php /* translators: %s : Theme Name */ printf(esc_html__( 'Thank you for installing %1$s. It is now installed and ready to use. Here are some useful links to get started: ', 'bloger' ), $this->theme_name);  ?></p>
						<hr/>
							...
							<div class="apwelcome-column col-demos">
								<div>
									<h3>
										...
										<?php printf(esc_html__( 'Starter site demos!', 'bloger' )) ?>
									</h3>
									<p><?php printf( wp_kses_post( 'You\'ve  got 1  great starter sites (Demos) to start with. By using one of these starter sites you\'ll get a great guidance on how to use the theme to its fullest and also save time to make your website. ', 'bloger' ), $this->theme_name, esc_url(admin_url( 'themes.php?page=welcome-page#demo_import' ) ) ); ?></p>
								</div>
								<div>
									<div class="button-wrapper">
										<a class="button button-primary button-hero install-now" href="<?php echo esc_url(admin_url( 'themes.php?page=welcome-page#demo_import' )) ?>"><?php esc_html_e( 'ready to use starter sites.', 'bloger' ); ?></a>
									</div>

									<?php printf( wp_kses_post( '<a class="options-page-btn notice-info-btn" href="%2$s">Or start setting up your theme now (without demo)!</a>', 'bloger' ), $this->theme_name, esc_url(admin_url( 'themes.php?page=welcome-page' ))  ); ?>

									<p><?php esc_html_e( 'You can also reach-out to us for any support during the theme setup and use.', 'bloger' ); ?></p>
									<a href="<?php echo esc_url('https://accesspressthemes.com/support/')?>" class=" ti-return-dashboard  button button-secondary  install-now" ><span><?php esc_html_e( 'Contact Support', 'bloger' ); ?></span></a>
							...
							<div class="apwelcome-column col-doc">
								<div>
									<h3>
										...
										<?php printf(esc_html__( 'Documentation', 'bloger' )) ?>
									</h3>
									<p><?php printf( wp_kses_post( 'How to use %1$s! Here we\'ve a full and detailed documentation that explains how to use %1$s in its best. ', 'bloger' ), $this->theme_name  ); ?></p>
									<a href="<?php echo esc_url($this->strings['doc_link']);?>" class=" button" ><span><?php esc_html_e( 'Full Documentation', 'bloger' ); ?></span></a>

								</div>
								<div class="apwelcome-doc-btn">
									<a href="#" class=" button" ><span><?php esc_html_e( 'Return to your Dashboard', 'bloger' ); ?></span></a>
								</div>
							</div>
						</div>
					</div>
			...
			public function welcome_screen() {
				...
				<div class="wrap about-wrap access-wrap">
								...
								<div class="welcome-section-wrapper is_loading">
									...
									<div class="notice-sidebar is_loading">
										<div class= "notice-sidebar-item">
											<h4><?php echo esc_html('Join in our social networks!','bloger') ?></h4>
											<p><?php echo esc_html__('Get connected, share your opinions and more via our social community: ', 'bloger'); ?></p>

											<p><a href="<?php echo esc_url('https://www.facebook.com/AccessPressThemes')?>"><?php echo esc_html('Join our Facebook Group','bloger') ?></a><?php echo esc_html__(' -to receive updates, offers and more.', 'bloger'); ?></p>

											<p><a href="<?php echo esc_url('https://www.youtube.com/channel/UCzD8I1moKISXDjAVr8dIf-w')?>"><?php echo esc_html('Subscribe our YouTube Channel','bloger') ?></a><?php echo esc_html__(' -for tutorials, videos and more.', 'bloger'); ?></p>

											<p><a href="<?php echo esc_url('https://twitter.com/apthemes')?>"><?php echo esc_html('Follow us on Twitter','bloger') ?></a><?php echo esc_html__(' -to stay updated. ', 'bloger'); ?></p>
										</div>
										<div class= "notice-sidebar-item">
											<h4><?php echo esc_html('Rate us!','bloger') ?></h4>
											<p><?php /* translators: %s : Theme Name */ printf(esc_html__('Are you enjoying %1$s? We would love to hear your feedback!', 'bloger'),$this->theme_name); ?></p>
											<a href="https://wordpress.org/support/theme/bloger/reviews/#new-post"><?php echo esc_html('Write a review','bloger') ?></a>
										</div>
									</div>
								</div>
							</div>
					...
					public function plugin_installer_callback(){
						...
						if ( ! current_user_can('install_plugins') ) {
							wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'bloger' ) );
						}
				...
				// Check our nonce, if they don't match then bounce!
						if (! wp_verify_nonce( $nonce, 'plugin_installer_nonce' )) {
							wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'bloger') );
						}
				...
				// Get Plugin Info
					...
					public function plugin_activation_callback() {
						...
						if(file_exists($plugin_file)) {
							...
							echo esc_html__('Plugin Does not Exists' , 'bloger');
						}
